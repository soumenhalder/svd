...
void efficiency_calculation_3(TString fname="../Input_rootfile/data_mumu_l3.root") {
TFile *file=new TFile("../histograms/final_histograms_l3_data_bhabha_for_systametics.root","RECREATE");
...
int count_background_cluster[2][2]={0};
...
for(int i=0;i<nentries1;i++){
   ...
   if(L_!=3) continue;
    if(!(U_ID>20 && U_ID<748 && V_ID>20 && V_ID<748)) continue;
   if(S==2) uv->Fill(U,V); 
   if(PXD_>0 && SVD_>0){
...
gStyle->SetOptFit(kTRUE);
 ...
 cout<<"intercept:31= "<<count_intercept[0]<<endl;
  ...
  cout<<"background cluster:31V= "<<count_background_cluster[0][0]<<"  cluster:31U= "<<count_background_cluster[0][1]<<endl; 
  cout<<"intercept:32= "<<count_intercept[1]<<endl;
  cout<<"cluster:32V= "<<count_cluster[1][0]<<"  cluster:32U= "<<count_cluster[1][1]<<endl;
  cout<<"background cluster:32V= "<<count_background_cluster[1][0]<<"  cluster:32U= "<<count_background_cluster[1][1]<<endl;
