...
TH1F *residual[5][2][10];
TH1F *residual_all[5][2];
TH1F *clusterposition[5][2];
TH1F *interceptposition[5][2];
TH1F *angleuv[5][2];
TH1F *intercept_e[5][2];
TH2F *residual_clspos[5][2];
TH2F *uv_id[5][2];
TH1F *intercept_error[5][5];
for(int p=0;p<5;p++){
  for(int t=0;t<2;t++){
    for(int q=0;q<10;q++){
    TString s= "61";
  ...
  TString s= "61";
    ...
    TString a,b,c,d,e,f,g,h,y,z,aa,yy;
   ...
   yy+=":Intercept error";
  intercept_e[p][t] =new TH1F(yy,yy,100,0,0.03);
  intercept_e[p][t]->GetXaxis()->SetTitle("error in cm");
  intercept_e[p][t]->GetXaxis()->SetTitleOffset(0.5);
  intercept_e[p][t]->GetXaxis()->SetTitleSize(0.07);
...
for(int i=0;i<nentries;i++){
   ...
   intercept_e[sensor_-1][side_]->Fill(error);
