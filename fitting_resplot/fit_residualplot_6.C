#include<cmath>
/*512V 513V 511V 514V*/
void fit_residualplot_6(TString fname="../Input_rootfile/data_bhabha_l5.root") {
using namespace RooFit;
RooRealVar x("resolution", " residual in cm",-0.07, 0.07);//0.05 for all except 512V
...
for(int i=0;i<nentries;i++){
   ...
   if(TMath::Abs(intercept-cls)>0.060) continue;////0.04 for all except 512V
  if(layer==5 && sensor==2 && svd>0 && pxd>0 && side==0){ 
...
RooPlot *myframe = x.frame(Name("512V"),Title("512V residual distribution"),Bins(60));
data->plotOn(myframe);
RooRealVar mean1("#mu_{1}","mean_{1}",0.000,-0.006,0.006);//0.002  for all except 512V
