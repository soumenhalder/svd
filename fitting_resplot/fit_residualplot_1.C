#include<cmath>
/*311V*/
void fit_residualplot_1(TString fname="../Input_rootfile/data_bhabha_l3.root") {
using namespace RooFit;
RooRealVar x("resolution", " residual in cm",-0.030, 0.030);
...
for(int i=0;i<nentries;i++){
   ...
   if(TMath::Abs(intercept-cls)>0.025) continue;
  if(layer==3 && sensor==2 && svd>0 && pxd>0 && side==0){ 
...
RooPlot *myframe = x.frame(Name("312V"),Title("312V residual distribution"),Bins(60));
data->plotOn(myframe);
RooRealVar mean1("#mu_{1}","mean_{1}",0.000,-0.002,0.002);
RooRealVar mean2("#mu_{2}","mean_{2}",0.001,-0.002,0.002);
