#include<cmath>
/*512U 513U 514U*/
void fit_residualplot_7(TString fname="../Input_rootfile/data_bhabha_l6.root") {
using namespace RooFit;
RooRealVar x("resolution", " residual in cm",-0.0500, 0.050);
...
for(int i=0;i<nentries;i++){
   ...
   if(TMath::Abs(intercept-cls)>0.030) continue;//0.020 for 614U: 0.025 for 512U:0.018 for 514U
  if(layer==6 && sensor==2 && svd>0 && pxd>0 && side==1){ 
...
RooPlot *myframe = x.frame(Name("612U"),Title("612U residual distribution"),Bins(60));
data->plotOn(myframe);
RooRealVar mean1("#mu_{1}","mean_{1}",0.000,-0.002,0.002);
RooRealVar mean2("#mu_{2}","mean_{2}",0.001,-0.005,0.005);
