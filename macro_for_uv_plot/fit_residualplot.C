#include<cmath>
void fit_residualplot(TString fname="Input_rootfile/data_mumu_l3.root") {
using namespace RooFit;
RooRealVar x("resolution", " residual in cm",-0.02, 0.02);
...
for(int i=0;i<nentries;i++){
  ...
  if(layer==3 && sensor==2 && svd>0 && pxd>0 && side==1){ 
     x.setVal(cls-intercept); 
...
RooPlot *myframe = x.frame(Name("311U"),Title("311U residual distribution"),Bins(60));
data->plotOn(myframe);
