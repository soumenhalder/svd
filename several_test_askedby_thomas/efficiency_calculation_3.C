...
TFile *file=new TFile("histograms/bhabha_uvl3.root","RECREATE");
...
TH1F *residual[2][2][10];
TH1F *angleuv[2][2];
TH2F *residual_clspos[2][2];
...
for(int i=0;i<nentries;i++){
   ...
   if(size_>9) count_clustersize[9]++;
   else count_clustersize[size_-1]++;
   if(size_>9) size_=10;
   if(layer!=3) continue;
   if(svd<6|| pxd<2) continue;
...
gStyle->SetOptFit(kTRUE);
 ...
 cout<<"intercept:31= "<<count_intercept[0]<<endl;
  ...
  for(int i=0;i<10;i++) 
     cout<<i+1<<"  "<<count_clustersize[i]<<endl;
file->Write();
}
